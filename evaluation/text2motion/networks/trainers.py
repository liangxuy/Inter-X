...
class CompTrainerV6(object):
    ...
    def forward(self, batch_data, tf_ratio, mov_len, eval_mode=False):
        ...
        '''Movement Encoding'''
        self.movements = self.mov_enc(self.motions).detach()
        # Initially input a mean vector
        mov_in = self.mov_enc(
            torch.zeros((self.motions.shape[0], self.opt.unit_length, self.motions.shape[-1]), device=self.device)
        ).squeeze(1).detach()
    ...
    def generate(self, word_emb, pos_ohot, cap_lens, m_lens, mov_len, dim_pose):
        ...
        # Initially input a mean vector
        mov_in = self.mov_enc(
            torch.zeros((word_emb.shape[0], self.opt.unit_length, dim_pose), device=self.device)
        ).squeeze(1).detach()
    ...
    def train(self, train_dataset, val_dataset, plot_eval):
        ...
        if self.opt.dataset_name == 't2m':
            ...
            schedule_len = 6
        elif self.opt.dataset_name == 'hhi':
            schedule_len = 10
        sub_ep = 0
        ...
        while invalid:
            ...
            for sub_epoch in range(sub_ep, self.opt.max_sub_epoch):
                ...
                if epoch % self.opt.eval_every_e == 0:
                    ...
                    fake_data = self.fake_motions
                    gt_data = self.motions
                    data = torch.cat([fake_data, reco_data, gt_data], dim=0).cpu().numpy()
                    captions = self.caption * 3
