...
from data.dataset import Text2MotionDataset, Text2MotionDatasetHHI, collate_fn
...
def plot_t2m(data, save_dir, captions):
    ...
    for i, (caption, joint_data) in enumerate(zip(captions, data)):
        ...
        plot_3d_motion(save_path + '.gif', paramUtil.t2m_kinematic_chain, joint, title=caption, fps=20)
...
if __name__ == '__main__':
    ...
    if opt.dataset_name == 't2m':
        ...
        opt.joints_num = 56
        dim_pose = opt.joints_num * 12 # 2 persons, rot6d
        dim_word = 300
        dim_pos_ohot = len(POS_enumerator)
        num_classes = 152 // opt.unit_length

        w_vectorizer = WordVectorizer(pjoin(opt.data_root, 'glove'), 'hhi_vab')
        split_file = pjoin(opt.data_root, 'splits', opt.split_file)
        opt.max_motion_length = 150
    else:
        raise KeyError('Dataset Does Not Exist')


    test_motion_file = pjoin(opt.motion_dir, 'test.h5')
    ...
    dataset = Text2MotionDatasetHHI(opt, split_file, w_vectorizer, test_motion_file)
